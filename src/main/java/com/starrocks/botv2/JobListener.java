...
public class JobListener extends RunListener<AbstractBuild> {
    ...
    public void onStarted(AbstractBuild build, TaskListener listener) {
        BotService service = getService(build, listener);
        if (service == null) {
            return;
        }
    ...
    private BotService getService(AbstractBuild build, TaskListener listener) {
        Map<Descriptor<Publisher>, Publisher> map = build.getProject().getPublishersList().toMap();
        for (Publisher publisher : map.values()) {
            if (publisher instanceof BotNotifier) {
                return ((BotNotifier) publisher).newBotService(build, listener);
            }
        }
        return null;
    }
